...
directory = "F:\\INVISAGE\\Q13S\\Column_issues\\Data\\20160523\\811w24#75R1\\RawImages"
...
file = os.path.join(directory,"image_imaging_gain1_0*.raw")
...
if bptn_profile:
    ...
    pylab.plot(list(range(len(ptn_col_split))),ptn_col_split)
    pylab.show()
    '''
    ...
    pylab.xlabel('Column')
    pylab.ylabel('Column PTN [DN]')
    pylab.plot(list(range(len(ptn_col))),ptn_col)
    pylab.show()
    '''
    ...
    for col_ptn in range(len(ptn_col)):
        ...
        pylab.plot(list(range(len(colx))),colx)
        #pylab.show()
